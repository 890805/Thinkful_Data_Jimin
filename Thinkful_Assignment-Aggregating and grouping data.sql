...
SELECT COUNT(DISTINCT main_category) as cnt_main_category, COUNT(DISTINCT category) as cnt_category
FROM ksprojects;

--15 and 158 unique values each
...
SELECT main_category, COUNT(category)
FROM ksprojects
...
SELECT category, AVG(goal)-AVG(usd_pledged) AS avg_diff, COUNT(*)
FROM ksprojects
